...
open class LocalSearchOptimizer<O : ObjectiveFunction>(val problem: Problem) : Optimizer<O> {
     ...
     */
    var maxSteps: Int = max(100, problem.binarySize)
     ...
     * tenure.
     */
    var tabuListSize: Int = Int.power2(min(problem.binarySize, 2))
        set(value) {
            field = if (value == 0) 0
     ...
     */
    var maxConsideration: Int = max(20, problem.binarySize / 5)
    ...
    override fun optimizeOrThrow(function: O, assumptions: IntCollection, guess: MutableInstance?): Instance {
        ...
        if (propagateAssumptions && assumptions.isNotEmpty()) {
            ...
            p = Problem(problem.binarySize, problem.unitPropagation(units, true))
        ...
        val adjustedMaxConsideration = max(2, min(maxConsideration, p.binarySize))
        ...
        for (restart in 1..restarts) {
            ...
            if (guess != null && restart == 1) {
                ...
                instance = instanceBuilder.create(p.binarySize)
            ...
            if (validator.totalUnsatisfied == 0 && (abs(bestValue - lowerBound) < eps || p.binarySize == 0))
            ...
            for (step in 1..maxSteps) {
                ...
                val ix: Int = if (pRandomWalk > rng.nextFloat()) {
                    ...
                    val itr: IntIterator = if (validator.totalUnsatisfied > 0) {
                        ...
                        n = min(adjustedMaxConsideration, p.binarySize)
                        if (p.binarySize > adjustedMaxConsideration)
                            OffsetIterator(1, IntPermutation(p.binarySize, rng).iterator())
                        else (1..p.binarySize).iterator()
                    }
