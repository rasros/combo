...
open class GeneticAlgorithmOptimizer<O : ObjectiveFunction>(val problem: Problem) : Optimizer<O> {
     ...
     */
    var candidateSize: Int = max(20, min(problem.binarySize * 5, 300))
     ...
     */
    var maxSteps: Int = max(500, problem.binarySize)
     ...
     */
    var stallSteps: Int = max(50, problem.binarySize / 4)
        ...
        val candidates = let {
            val validators: Array<Validator> = Array(candidateSize) {
                if (guess != null) Validator(p, guess.copy(), assumption)
                    ...
                    val instance = instanceBuilder.create(p.binarySize)
        ...
        for (restart in 1..restarts) {
            ...
            for (i in 0 until candidateSize) {
                if (i in keep) {
                    ...
                    val newInstance = instanceBuilder.create(p.binarySize)
