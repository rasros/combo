...
open class GAOptimizer<O : ObjectiveFunction>(val problem: Problem) : Optimizer<O> {
    ...
    override fun optimizeOrThrow(function: O, assumptions: Literals): Instance {
        ...
        val population: Array<TrackingInstance> = Array(candidateSize) {
        ...
        val candidates = let {
            ...
            val scores = DoubleArray(candidateSize) {
                ...
                if (abs(s - lowerBound) < eps && population[it].totalUnsatisfied == 0)
                    return population[it].instance
                s
            }
        ...
        for (restart in 1..restarts) {
            ...
            for (i in 0 until candidateSize) {
                if (i in keep) {
                    ...
                    population[i] = trackingInstanceFactory.build(instanceFactory.create(problem.nbrVariables), assumptions, initializer, function, randomSequence.next())
                    @Suppress("UNCHECKED_CAST")
                    (candidates.instances as Array<MutableInstance>)[i] = population[i]
                    candidates.update(i, 0, score(population[i]))
                }
            }
        }

        for (i in 0 until candidateSize)
            if (population[i].totalUnsatisfied == 0)
                return population[i].instance
